...
public:
    ...
    void train(std::vector<Test> tests)
    {
        ...
        int i;
		for(i = 0; i < 10000 && success_rate < 1.0; ++i) {
...
private:
	...
	friend void test2(const std::vector<Test>& data);
};
...
void test2(const std::vector<SNeuron::Test>& data)
{
	SNeuron neuron;
	for(int i = 0; i < data[0].inputs.size(); ++i)
		neuron.add_input(new SNeuron());
	...
	{
		for(int j = 0; j < data[i].inputs.size(); ++j)
			std::cout << data[i].inputs[j] * std::numeric_limits<float>::max() << ' ';
		neuron.test(data[i].inputs);
		std::cout << "| " << neuron.output * std::numeric_limits<float>::max() << std::endl;
	}
...
void pic_test()
{
	QImage img("mnist_100_digits.png");
	...
	struct Img_Data { QImage img; int result; };
	std::vector<Img_Data> images;

	for(int x = 2; x < img.width(); x+=step_x) {
		for(int y = 4; y < img.height(); y+=step_y) {
			images.push_back(img.copy(x, size_x, y, size_y));
		}
	}

	const int numbers_x = 10;
	const int numbers_y = 10;
	for(int x = 0; x < numbers_x; ++x) {
		for(int y = 0; y < numbers_y; ++y) {
