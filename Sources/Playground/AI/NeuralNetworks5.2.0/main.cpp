...
int main(void)
{
	...
	NeuralNetwork nw;
	std::shared_ptr<Layer> l1 = nw.add_layer();
	...
	for(int i = 0; i < train_pictures.size() && i < train_numbers.size(); ++i) {
		...
		const std::vector<float>* testing =  nw.train(data, results);
	...
	for(int i = 0; i < test_pictures.size() && i < test_numbers.size(); ++i) {
		...
		const std::vector<float>* testing = nw.process(data);
	...
	std::cout << "False: " << 10000 - float(successes) << "/" << test_pictures.size() << " = " << (10000 - float(successes)) / test_pictures.size() * 100 << "%" << std::endl;
	for(int i = 0; i < success_count.size(); ++i)
	{
