...
template < class T, class = typename ::std::enable_if<::std::is_arithmetic<T>::value && !(::std::is_same<char, T>::value)>::type>
...
/// for arithmetic types that are not chars
/// Note: only for integral types
template < class T, class = typename ::std::enable_if<::std::is_integer<T>::value && !(::std::is_same<char, T>::value)>::type>
bool operator ==(const ::std::string& _rcLeftString, const T& _rcRightValue)
{
    return _rcLeftString == ::std::to_string(_rcRightValue);
}
/// for arithmetic types that are not chars
/// Note: only for integral types
template < class T, class = typename ::std::enable_if<::std::is_integer<T>::value && !(::std::is_same<char, T>::value)>::type>
bool operator ==(const T& _rcLeftValue, const ::std::string& _rcRightString)
{
    return ::std::to_string(_rcLeftValue) == _rcRightString;
}
